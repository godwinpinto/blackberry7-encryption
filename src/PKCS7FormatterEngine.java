...
public class PKCS7FormatterEngine implements BlockFormatterEngine {
	...
	public int formatAndEncrypt(byte[] input, int inputOffset, int inputLength, byte[] output, int outputOffset) throws MessageTooLongException,
	CryptoTokenException {
		return formatAndEncrypt(input, inputOffset, inputLength, output, outputOffset, false);
	}
	...
	throws MessageTooLongException, CryptoTokenException {
		if (lastBlock) {
			int padding = _encryptorEngine.getBlockLength() - (inputLength % _encryptorEngine.getBlockLength());
			int paddedInputLength = inputLength + padding;
			byte[] paddedInput = new byte[paddedInputLength];
			for (int i = 0; i < paddedInputLength; i++) {
				if (i < inputLength) paddedInput[i] = input[i];
				else paddedInput[i] = (byte)(padding);
			}
